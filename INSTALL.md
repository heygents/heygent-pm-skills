...
```
✔ Successfully added marketplace: heygent
```
...
You should see `heygent` listed alongside any other marketplaces you have.
...
(Alternatively, from inside a Claude Code session: `/plugin install heygent-pm-skills@heygent`.)

**Expected output:**

```
✔ Successfully installed plugin: heygent-pm-skills@heygent (scope: user)
```
...
You should see `heygent-pm-skills@heygent` with status `✔ enabled`.

**What just happened:** Claude Code cloned this repo into `~/.claude/plugins/cache/heygent/heygent-pm-skills/<version>/`. Skills under `skills/*/SKILL.md` are now discoverable as slash commands (`/00-onboarding` for setup, `/01-customer-discovery` through `/19-ds-extract` for PM and design-system work) and auto-triggered by the model when relevant.
...
```bash
bash ~/.claude/plugins/cache/heygent/heygent-pm-skills/1.0.0/bin/bootstrap.sh
```
...
cp ~/.claude/plugins/cache/heygent/heygent-pm-skills/1.0.0/templates/CLAUDE.md.template ./CLAUDE.md
cp ~/.claude/plugins/cache/heygent/heygent-pm-skills/1.0.0/templates/Knowledge/workspace-tools.md.template Knowledge/workspace-tools.md
ls -la Knowledge
```
...
- A tour of skills 01–19 and a **personalized recommendation** for what to run first
...
> Skip this step entirely if you don't plan to use skill 11. The other skills work without it.
...
```bash
bash ~/.claude/plugins/cache/heygent/heygent-pm-skills/1.0.0/bin/setup-skill-11.sh
```
...
```bash
npm --prefix ~/.claude/plugins/cache/heygent/heygent-pm-skills/1.0.0/skills/11-competitor-feature-analysis run competitor-login -- --competitor <slug> --verify "<url>"
```
...
```bash
npm --prefix ~/.claude/plugins/cache/heygent/heygent-pm-skills/1.0.0/skills/11-competitor-feature-analysis run competitor-research -- --feature "<name>"
```
...
claude plugin update heygent-pm-skills@heygent
```

(or inside Claude Code: `/plugin update heygent-pm-skills@heygent`).
...
```bash
claude plugin uninstall heygent-pm-skills
claude plugin marketplace remove heygent
```
...
The marketplace's `source.url` is HTTPS, so this shouldn't happen on a clean install. If you see this, check that your `marketplace.json` cache wasn't stale — run `claude plugin marketplace update heygent` and re-install.

**Plugin installed but `/01-...` slash commands don't appear**
Restart Claude Code. Slash commands from new plugins are picked up on startup. If they still don't appear, check `claude plugin list` shows the plugin as `✔ enabled` and re-run `claude plugin enable heygent-pm-skills`.
...
CloakBrowser only ships macOS and Windows binaries today. Skill 11 won't work on Linux. The other skills work everywhere.
...
Restart Claude Code after updating the plugin. Run `claude plugin list` to confirm `heygent-pm-skills` is enabled, then `claude plugin update heygent-pm-skills@heygent`.
...
| What | Where |
|------|-------|
| Plugin code (read-only) | `~/.claude/plugins/cache/heygent/heygent-pm-skills/1.0.0/` |
...
- File issues at [github.com/heygents/heygent-pm-skills/issues](https://github.com/heygents/heygent-pm-skills/issues).
