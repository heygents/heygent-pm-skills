...
3. Awareness of all 19 workflow skills and which to run first
4. A saved summary at `Knowledge/onboarding-summary.md` with a personalized next step

**Assumes:** the `heygent-pm-skills` plugin is already installed. Do not walk through `claude plugin install`.
...
Use this table when explaining what the user has. Skills 01–19 ship with the plugin; this skill is `00`.
...
**Typical flow:** 01 discovery → 02–04 planning → 05 PRD → 06 tech plan → 07–09 review → 10 learn. Skill 18 can follow 05 PRD to break the spec into a Jira Epic with User Stories. Skills 11–14 (competitor analysis, knowledge ingest, validation storyboard, Mixpanel analysis) are standalone, run anytime. Skills 15–17 are a design-system toolkit — build, compliance audit, and prototype → Figma — and read DS context from `Knowledge/Design/`. That folder starts as a template: run **19 `/19-ds-extract`** once with your brand assets (screenshots, CSS, Figma, brand PDF) to generate it; until then 15–17 stop and point you there.
...
> "Next I'll set up your **workspace** — the folder where HeyGent keeps your PM work."
...
2. If `CLAUDE.md` or `Knowledge/workspace-tools.md` is missing, bootstrap:
   - Prefer running the plugin bootstrap script if you can resolve its path:
     `bash ~/.claude/plugins/cache/heygent/heygent-pm-skills/*/bin/bootstrap.sh`
...
```markdown
# HeyGent onboarding complete — <YYYY-MM-DD>
