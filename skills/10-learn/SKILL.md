---
name: 10-learn
description: Captures HeyGent learnings anytime—ship retros (PRD/TDD updates) or process/skill tuning from user and model feedback—and writes durable notes under Learnings/. Use for retro, postmortem, lessons learned, skill improvements, or /10-learn.
---

# Learn (retro + continuous improvement)

Make HeyGent **better every iteration**: capture what happened, why, and what to change—in **product docs**, **technical docs**, **Learnings notes**, and optionally **other skills/commands**.
...
2. **Identify the implicated skill(s) by slash command / name** (e.g. `/05-create-prd`). HeyGent skills are installed **read-only** from the plugin (under `~/.claude/plugins/cache/...`, replaced on every update), so you can't edit them in place — capture *what should change*, don't try to patch files.
...
## Affected skills
- `/NN-skill-name` (the installed HeyGent skill this concerns)
