#!/usr/bin/env bash
# Validate a HeyGent PM Skills workspace.
#
# Checks required folders and flags unfilled placeholders in CLAUDE.md
# and Knowledge/workspace-tools.md.
#
# Usage (from workspace root):
#   bash ~/.claude/plugins/cache/heygent/heygent-pm-skills/<version>/bin/validate-workspace.sh
#
...
# Design system config (skills 15–17)
if [ -f "Knowledge/Design/ds-config.md" ] && grep -q '\[FILL' Knowledge/Design/ds-config.md 2>/dev/null; then
  warnings+=("Knowledge/Design/ds-config.md is still a template — run /19-ds-extract before using skills 15–17")
fi
